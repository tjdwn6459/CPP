#include <stdio.h>
#include <stdlib.h>
#include <conio.h>//키보드에 관련된 기능을 하는 함수 
#include "cursor.h"
...
public:
	...
	void Run() {
		...
		sprintf_s(Mes, "%d 속도로 %s쪽 %d도 방향으로 %s진중  ",
...
int main()
{
	...
	for (;;) {
		gotoxy(10, 10);//콘솔창의 좌표를 나타낸다 
		printf("1~5: 기어변속, 6:후진 기어 , 0:기어 중립");
		gotoxy(10, 11);
		printf("위:엑셀, 아래:브레이크, 좌우:핸들, Q:종료");
		if (_kbhit())//
		{
			ch = _getch();//정보를 읽어와서 ch에 저장 
			if (ch == 0xE0 || ch == 0) {
				ch = _getch();
				switch (ch) {
				case 78:
